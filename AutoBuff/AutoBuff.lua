...
cSpellList = {
	...
	[AUTOBUFF_ABILITY_WATER_SHIELD] =      { ["type"] = "self" },
	[AUTOBUFF_ABILITY_EARTH_SHIELD] =      { ["type"] = "self" },
...
local cShamanShields = {
	[AUTOBUFF_ABILITY_LIGHTNING_SHIELD] = true,
	[AUTOBUFF_ABILITY_WATER_SHIELD] = true,
	[AUTOBUFF_ABILITY_EARTH_SHIELD] = true,
};
	 ...
	 (not UnitIsDeadOrGhost("player")) then
		...
		for i=0,23 do -- Cycle currently active buffs
			...
			if (buffName) then
				...
				if (vClass == string.lower(AUTOBUFF_CLASS_SHAMAN)) then
					if (cShamanShields[buffName]) then
						-- A shaman shield is already active
						polyShield = true;
					end
				end
					...
					if (AutoBuff_CanCast(iName, trigger, iBuff[iName])) then
						...
						if (not several) then
							...
							if (AutoBuff_Cooldown(id) == 0) then
								...
								-- CASTING ROUTINE
								if (not polyShadowform and not polyCatCast and not polyMoonCast and not polyRogue and not polyPaladin and not polyRejuv and not polyShield) then
